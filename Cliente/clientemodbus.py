...
class ClienteMODBUS():
    ...
    def atendimento(self):
        ...
        try:
            atendimento = True
            while atendimento:
                sel = input("Deseja realizar uma leitura, escrita ou configuração? (1- Leitura | 2- Escrita | 3- Configuração | 4 - Escrita Float | 5 - Leitura Float | 6 - Sair): ")
                ...
                if sel == '1':
                    ...
                    valor = float(input(f"Digite o valor que deseja escrever: "))
                    builder = BinaryPayloadBuilder(byteorder=Endian.LITTLE, wordorder=Endian.LITTLE)
                    builder.add_32bit_float(valor)
                    payload = builder.to_registers()
                    self._cliente.write_multiple_registers(addr, payload)
